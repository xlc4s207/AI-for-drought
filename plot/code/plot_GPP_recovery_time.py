...
def plot_spatial_distribution(data1, bounds1, data2, bounds2, output_path):
    ...
    cbar1.set_label('GPP Recovery Time (days)', fontsize=12)
    
    # 设置标题和标签
    ax1.set_title('SMrz GPP Recovery Time (Mean)', fontsize=14, fontweight='bold', pad=15)
    ...
    cbar2 = plt.colorbar(im2, ax=ax2, orientation='horizontal',
                         pad=0.05, shrink=0.8, aspect=30)
    cbar2.set_label('GPP Recovery Time (days)', fontsize=12)
    
    # 设置标题和标签
    ax2.set_title('SMs GPP Recovery Time (Mean)', fontsize=14, fontweight='bold', pad=15)
...
def main():
    ...
    # 输入文件路径
    smrz_file = '/home/xulc/flash_drought/process/flash_draught_GPP_RECO/SMrz_GPP_results/tif/SMrz_GPP_t_recover_mean.tif'
    sms_file = '/home/xulc/flash_drought/process/flash_draught_GPP_RECO/SMs_GPP_results/spatial_tif/SMs_GPP_t_recover_mean.tif'
    
    # 输出目录和文件
    output_dir = '/home/xulc/flash_drought/process/plot/jpg'
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, 'GPP_recovery_time_comparison.jpg')
    
    print("="*80)
    print("GPP恢复时间空间分布图绘制")
    print("="*80)
    
    # 读取数据
    print(f"\n读取SMrz GPP恢复时间数据: {smrz_file}")
