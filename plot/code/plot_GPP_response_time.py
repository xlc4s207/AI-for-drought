...
def plot_spatial_distribution(data1, bounds1, data2, bounds2, output_path):
    ...
    # 设置标题和标签
    ax1.set_title('SMrz GPP Response Time (Mean)', fontsize=14, fontweight='bold', pad=15)
    ...
    # 设置标题和标签
    ax2.set_title('SMs GPP Response Time (Mean)', fontsize=14, fontweight='bold', pad=15)
...
def main():
    ...
    # 输入文件路径
    smrz_file = '/home/xulc/flash_drought/process/flash_draught_GPP_RECO/SMrz_GPP_results/tif/SMrz_GPP_t_response_mean.tif'
    sms_file = '/home/xulc/flash_drought/process/flash_draught_GPP_RECO/SMs_GPP_results/spatial_tif/SMs_GPP_t_response_mean.tif'
    
    # 输出目录和文件
    output_dir = '/home/xulc/flash_drought/process/plot/jpg'
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, 'GPP_response_time_comparison.jpg')
    
    print("="*80)
    print("GPP响应时间空间分布图绘制")
    print("="*80)
    
    # 读取数据
    print(f"\n读取SMrz GPP响应时间数据: {smrz_file}")
