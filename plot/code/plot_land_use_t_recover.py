...
# -*- coding: utf-8 -*-
"""
绘制不同土地利用类型的SMrz和SMs GPP恢复时间空间分布图（带海岸线）
"""
...
def main():
    ...
    # 输出目录
    output_dir = '/home/xulc/flash_drought/process/plot/jpg/land_use_t_recover'
    os.makedirs(output_dir, exist_ok=True)
    
    print("="*80)
    print("土地利用类型GPP恢复时间空间分布图绘制（带海岸线）")
    print("="*80)
    
    # 获取所有t_recover文件
    smrz_files = sorted(glob.glob(os.path.join(smrz_dir, 'Class_*_t_recover_mean.tif')))
    ...
    for smrz_file in smrz_files:
        ...
        # 输出文件名
        output_file = os.path.join(output_dir, f'Class_{class_num}_t_recover_comparison.jpg')
