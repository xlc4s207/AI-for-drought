...
# -*- coding: utf-8 -*-
"""
绘制不同季节的SMrz和SMs GPP恢复时间空间分布图（带海岸线）
"""
...
def plot_seasonal_comparison(season, smrz_file, sms_file, output_path):
    ...
    # 设置标题
    ax1.set_title(f'SMrz - {season}\nGPP Recovery Time', fontsize=14, fontweight='bold', pad=15)
...
def main():
    ...
    # 输出目录
    output_dir = '/home/xulc/flash_drought/process/plot/jpg/seasonal_t_recover'
    os.makedirs(output_dir, exist_ok=True)
    
    print("="*80)
    print("季节性GPP恢复时间空间分布图绘制（带海岸线）")
    print("="*80)
    ...
    for season in seasons:
        # 构建文件路径
        smrz_file = os.path.join(smrz_dir, f'SMrz_GPP_{season}_t_recover.tif')
        sms_file = os.path.join(sms_dir, f'{season}_t_recover.tif')
        ...
        # 输出文件名
        output_file = os.path.join(output_dir, f'{season}_t_recover_comparison.jpg')
