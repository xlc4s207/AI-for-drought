...
RESULT_DIR = os.path.join(BASE_DIR, "result", "SMrz_result_v5.2")
# 骤旱爆发期时间约束
MIN_ONSET_DAYS = 5
MAX_ONSET_DAYS = 20
...
def parse_args():
    parser = argparse.ArgumentParser(description='全球干旱检测程序 v5.2 (深度内存优化)')
...
def init_netcdf_files(result_dir, n_lat, n_lon, lat_sub, lon_sub):
    ...
    for event_type in ['total', 'flash', 'nonflash']:
        ...
        with nc.Dataset(filepath, 'w', format='NETCDF4') as ds:
            ...
            ds.title = f'{event_type.capitalize()} Drought Events Details v5.2'
            ds.source = f'GLEAM SMrz data ({YEARS[0]}-{YEARS[-1]})'
            ds.algorithm = 'Two-step Method v5.2 (Memory Optimized)'
...
def main():
    ...
    print("="*70)
    print("   全球干旱检测 v5.2 - 深度内存优化版 (SMrz 根系层)")
    print("="*70)
