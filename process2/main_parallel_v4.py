...
"""
骤旱检测主程序 - 性能优化版 v4
基于 v3 版本，主要修正 (参考 flash_drought_SMrz1.md):
...
# 数据路径配置 - SMrz 根系层土壤湿度 (合并后的45年数据)
BASE_DIR = "/home/xulc/flash_drought/gleam"
SM_FILE = "/data/GLEAM/SMrz_45years.nc"  # 1980-2024 合并文件
RESULT_DIR = os.path.join(BASE_DIR, "result", "SMrz_result_v4")
...
def parse_args():
    parser = argparse.ArgumentParser(description='全球骤旱检测程序 - 优化版v4')
...
def calculate_backward_moving_average_by_year(data_2d, dates, window=MOVING_WINDOW):
    """
    修正3: 按年重置的后向滑动平均 (避免跨年污染)
    
    参数:
    - data_2d: shape (T, n_lon) 的数据
    - dates: list of (year, doy)
    - window: 窗口大小
    
    返回:
    - ma_2d: shape (T, n_lon) 的滑动平均
    """
    min_valid = window // 2
...
def build_doy_indices(years):
    ...
    """
    dates = []
    doy_to_indices = {doy: [] for doy in range(1, 366)}  # 只到 365
    doy_to_indices_for_ref = {doy: [] for doy in range(1, 366)}  # 基准期索引
    ...
    for year in years:
        ...
        for doy in range(1, n_days + 1):
            ...
            dates.append((year, doy_mapped))  # 存储映射后的 doy
...
def calculate_percentiles_batch(ma_2d, doy_to_indices_for_ref):
    """
    修正1: 批量计算每个 DOY 的 P20/P40 阈值 (只使用基准期 1981-2010)
    
    参数:
    - ma_2d: shape (T, n_lon) 的滑动平均数据
    - doy_to_indices_for_ref: DOY -> 基准期索引列表的映射
    
    返回:
    - p20_2d: shape (366, n_lon) - 每个 DOY 每列的 P20
    - p40_2d: shape (366, n_lon) - 每个 DOY 每列的 P40
    """
    n_lon = ma_2d.shape[1]
    p20_2d = np.full((366, n_lon), np.nan, dtype=np.float64)
    p40_2d = np.full((366, n_lon), np.nan, dtype=np.float64)
    
    for doy in range(1, 366):  # 修正2: 只到 365
        indices = doy_to_indices_for_ref.get(doy)
        if indices is None or len(indices) == 0:
            continue
        
        # 只提取基准期 (1981-2010) 的数据
...
def analyze_pixel_optimized(sm_ma, p20_arr, p40_arr, dates):
    ...
    修正4: NORMAL 状态遇 NaN 时重置 wet_start_idx
    """
    ...
    for i in range(n):
        year, doy = dates[i]
        # 注意: doy 已经在 build_doy_indices 中被映射 (366->365)
        
        # 处理 NaN
        if np.isnan(sm_ma[i]):
            # 修正4: NORMAL 状态下遇到 NaN，重置 wet_start_idx
            if state == 'NORMAL':
                wet_start_idx = None
            elif state == 'DROUGHT':
...
def process_single_row(args):
    ...
    try:
        ...
        # 3. 使用基准期 (1981-2010) 计算 P20/P40 (修正1)
...
def main():
    ...
    print("="*60)
    print("   全球骤旱(Flash Drought)检测 - 修正版 v4")
    print("="*60)
    ...
    # 保存事件详情 NetCDF
    nc_path = os.path.join(RESULT_DIR, "flash_drought_events_details_v4.nc")
    with nc.Dataset(nc_path, 'w', format='NETCDF4') as ds:
        ...
        ds.title = 'Flash Drought Events Details v4 (Corrected)'
    ...
    print(f"  保存: flash_drought_events_details_v4.nc")
