...
RESULT_DIR = os.path.join(BASE_DIR, "result", "SMrz_result_v3")
...
def parse_args():
    parser = argparse.ArgumentParser(description='全球骤旱检测程序 - 优化版v3')
...
def build_doy_indices(years):
    ...
    返回: dates_list, doy_to_indices (dict: doy -> list of indices)
    """
    dates = []
...
def calculate_percentiles_batch(ma_2d, doy_to_indices):
    ...
    - doy_to_indices: DOY -> 索引列表的映射
    
    返回:
    - p20_2d: shape (367, n_lon) - 每个 DOY 每列的 P20
    - p40_2d: shape (367, n_lon) - 每个 DOY 每列的 P40
    """
    n_lon = ma_2d.shape[1]
    ...
    for doy in range(1, 367):
        ...
        # 提取该 DOY 的所有年份数据
        doy_data = ma_2d[indices, :]  # shape: (n_years, n_lon)
...
def analyze_pixel_optimized(sm_ma, p20_arr, p40_arr, dates):
    ...
    参数:
    - sm_ma: 1D array, 该像元的滑动平均序列
    - p20_arr: 1D array (367,), 该像元每个 DOY 的 P20
    - p40_arr: 1D array (367,), 该像元每个 DOY 的 P40
    - dates: list of (year, doy)
    
    返回:
    - events, total_count, yearly_counts, is_valid
    """
    # 检查有效数据比例
...
def main():
    ...
    print("="*60)
    print("   全球骤旱(Flash Drought)检测 - 优化版 v3")
    print("="*60)
    ...
    print()
    print("[优化内容]")
    print("  ✓ 向量化滑动平均 (cumsum)")
    print("  ✓ 批量计算 P20/P40")
    print("  ✓ 预计算 DOY 索引")
    print("  ✓ event_count 用 -1 标记无效像元")
    ...
    # 保存事件详情 NetCDF
    nc_path = os.path.join(RESULT_DIR, "flash_drought_events_details_v3.nc")
    with nc.Dataset(nc_path, 'w', format='NETCDF4') as ds:
        ...
        ds.title = 'Flash Drought Events Details v3 (Optimized)'
        ds.source = f'GLEAM SMrz data ({YEARS[0]}-{YEARS[-1]})'
        ds.algorithm = 'Time Constraint Method v3 (Vectorized)'
    
    print(f"  保存: flash_drought_events_details_v3.nc")
