# -*- coding: utf-8 -*-
"""
骤旱检测核心模块 v5.4 - 性能与内存深度优化版
==================================================
基于 v5.3 的核心改进:
1. Pool 全局复用: 整个处理过程只创建一次进程池
2. NC I/O 优化: 检测连续时间索引使用切片读取
3. 内存优化: 消除 chunk_events 缓冲区 (~720MB → ~15MB/行)
4. 海洋行轻量返回: 减少约70%的无效序列化传输
5. 百分位数计算优化: 预构建窗口索引, 合并分位数调用
6. 年度数据紧凑返回: 3个数组替代135个数组
7. 代码模块化: 公共逻辑抽取, SMs/SMrz 共享

保留 v5.3 的所有算法特性:
- 两步法: 先识别所有干旱事件, 再分类 (flash vs non-flash)
- 三类输出: Total / Flash / Non-flash drought
- 5天窗口百分位数计算
- 基准期: 1981-2010
...
def process_single_row(args):
    ...
    try:
        ...
        # V5.4: 年度数据用紧凑 2D 数组 (n_years, n_lon) 替代 dict of arrays
        yearly_total = np.full((n_years, n_lon), np.nan, dtype=np.float32)
        yearly_flash = np.full((n_years, n_lon), np.nan, dtype=np.float32)
        yearly_nonflash = np.full((n_years, n_lon), np.nan, dtype=np.float32)
        ...
        for j in range(n_lon):
            ...
            # V5.4: 用紧凑数组索引替代 dict 遍历
            for e in all_evts:
                yi = year_to_idx.get(e['drought_start_year'])
                if yi is not None:
                    yearly_total[yi, j] = (yearly_total[yi, j] + 1) if not np.isnan(yearly_total[yi, j]) else 1

            for e in flash_evts:
                yi = year_to_idx.get(e['drought_start_year'])
                if yi is not None:
                    yearly_flash[yi, j] = (yearly_flash[yi, j] + 1) if not np.isnan(yearly_flash[yi, j]) else 1

            for e in nonflash_evts:
                yi = year_to_idx.get(e['drought_start_year'])
                if yi is not None:
                    yearly_nonflash[yi, j] = (yearly_nonflash[yi, j] + 1) if not np.isnan(yearly_nonflash[yi, j]) else 1

            # 初始化已访问像素的年度数据为 0 (而非 NaN)
            for yi_init in range(n_years):
                if np.isnan(yearly_total[yi_init, j]):
                    yearly_total[yi_init, j] = 0
                if np.isnan(yearly_flash[yi_init, j]):
                    yearly_flash[yi_init, j] = 0
                if np.isnan(yearly_nonflash[yi_init, j]):
                    yearly_nonflash[yi_init, j] = 0
            ...
            valid_count += 1
            total_event_count += int(row_total[j])
            flash_event_count += int(row_flash[j])
            nonflash_event_count += int(row_nonflash[j])
        ...
        return {
            ...
            'yearly_flash': yearly_flash,
            'yearly_nonflash': yearly_nonflash,
...
def init_netcdf_files(result_dir, n_lat, n_lon, lat_sub, lon_sub, sm_var, source_label):
    ...
    for event_type in ['total', 'flash', 'nonflash']:
        ...
        with nc.Dataset(filepath, 'w', format='NETCDF4') as ds:
            ...
            ds.title = f'{event_type.capitalize()} Drought Events Details v5.4'
            ds.source = f'MERRA2 {sm_var} data ({YEARS[0]}-{YEARS[-1]})'
            ds.algorithm = 'Two-step Method v5.4 (Optimized, 5-day Window Percentile)'
...
def write_row_events_to_nc(nc_handles, result, global_row, n_lon):
    ...
    for event_type in ['total', 'flash', 'nonflash']:
        ...
        del event_count_row, osy, osd, dsy, dsd, dey, ded, ond, dur, odr, ort, ity
    ...
    try:
        ...
        with Pool(processes=n_workers, initializer=init_worker,
            ...
            for chunk_idx in range(n_chunks):
                ...
                # V5.4: 年度数据用 (n_years, chunk_size, n_lon) 数组
                chunk_yearly = {
                    'total': np.full((n_years, actual_chunk_size, n_lon), np.nan, dtype=np.float32),
                    'flash': np.full((n_years, actual_chunk_size, n_lon), np.nan, dtype=np.float32),
                    'nonflash': np.full((n_years, actual_chunk_size, n_lon), np.nan, dtype=np.float32)
                }
                ...
                for result in pool.imap_unordered(process_single_row, tasks, chunksize=4):
                    ...
                    # V5.4: 更新年度数据 (紧凑数组格式)
                    chunk_yearly['total'][:, row_idx, :] = result['yearly_total']
                    chunk_yearly['flash'][:, row_idx, :] = result['yearly_flash']
                    chunk_yearly['nonflash'][:, row_idx, :] = result['yearly_nonflash']
