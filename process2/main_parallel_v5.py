...
分类规则:
  - Flash: 存在 onset_start 且 5 <= onset_days <= 30
  - Non-flash:
    - slow-onset: 存在 onset_start 但 onset_days > 30
...
RESULT_DIR = os.path.join(BASE_DIR, "result", "SMrz_result_v5")
...
def parse_args():
    parser = argparse.ArgumentParser(description='全球干旱检测程序 v5 (完整分类)')
...
def read_row_from_merged(lat_idx, lon_start, lon_end, sm_ds):
    """从合并后的45年NC文件读取一行数据"""
    row_data = sm_ds.variables['SMrz'][:, lat_idx, lon_start:lon_end]
...
def main():
    ...
    print("="*70)
    print("   全球干旱检测 v5 - 完整分类版 (SMrz 根系层)")
    print("="*70)
    ...
    # 保存频率 TIFF
    print("  保存总频率...")
    save_tiff(freq_total, f"total_drought_frequency_{YEARS[0]}_{YEARS[-1]}.tif", lat_sub, lon_sub)
    save_tiff(freq_flash, f"flash_drought_frequency_{YEARS[0]}_{YEARS[-1]}.tif", lat_sub, lon_sub)
    save_tiff(freq_nonflash, f"nonflash_drought_frequency_{YEARS[0]}_{YEARS[-1]}.tif", lat_sub, lon_sub)
    
    print("  保存年度频率...")
    for year in YEARS:
        save_tiff(yearly_total[year], f"total_drought_{year}.tif", lat_sub, lon_sub)
        save_tiff(yearly_flash[year], f"flash_drought_{year}.tif", lat_sub, lon_sub)
        save_tiff(yearly_nonflash[year], f"nonflash_drought_{year}.tif", lat_sub, lon_sub)
    
    # 保存事件详情 NetCDF
    print("  保存事件详情 NetCDF...")
    save_events_netcdf(os.path.join(RESULT_DIR, "total_drought_events_v5.nc"),
                       total_events_data, n_lat, n_lon, lat_sub, lon_sub, "Total Drought")
    save_events_netcdf(os.path.join(RESULT_DIR, "flash_drought_events_v5.nc"),
                       flash_events_data, n_lat, n_lon, lat_sub, lon_sub, "Flash Drought")
    save_events_netcdf(os.path.join(RESULT_DIR, "nonflash_drought_events_v5.nc"),
                       nonflash_events_data, n_lat, n_lon, lat_sub, lon_sub, "Non-flash Drought")
