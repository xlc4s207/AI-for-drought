# -*- coding: utf-8 -*-
"""
骤旱检测主程序 v5.3 - 5天窗口百分位数优化版 (SMrz 根系层)
==================================================
核心改进:
...
RESULT_DIR = os.path.join(BASE_DIR, "result", "SMrz_result_v5.3")
...
def parse_args():
    parser = argparse.ArgumentParser(description='全球干旱检测程序 v5.3 (5天窗口百分位数)')
...
def build_doy_indices(years):
    ...
    返回:
        dates: [(year, doy), ...]
        doy_to_indices: {doy: [idx1, idx2, ...]} 所有年份
        doy_to_indices_for_ref: {doy: [idx1, idx2, ...]} 仅基准期
    """
    dates = []
...
def main():
    ...
    print("="*70)
    print("   全球干旱检测 v5.3 - 5天窗口百分位数优化版 (SMrz 根系层)")
    print("="*70)
    ...
    for chunk_idx in range(n_chunks):
        ...
        def init_chunk_events_data(size):
            return {
                ...
                'days_below_p20': np.full((MAX_EVENTS_PER_PIXEL, size, n_lon), -1, dtype=np.int16),
                ...
                def save_row_events(events_list, events_data, row):
                    for j, evts in events_list:
                        ...
                        for k, e in enumerate(evts):
                            ...
                            events_data['days_below_p20'][k, row, j] = e['days_below_p20']
