...
# 数据路径配置 - SMrz 根系层土壤湿度
BASE_DIR = "/home/xulc/flash_draught"
SM_FILE = "/datahuge/GLEAM/SMrz_45years.nc"
RESULT_DIR = os.path.join(BASE_DIR, "Flash_result", "SMrz_result_v6_global")
...
# 向量化写入参数
BATCH_WRITE_SIZE = 50    # 批量写入缓冲区大小 (行数) - 增大以提升性能
...
def parse_args():
    parser = argparse.ArgumentParser(description='全球干旱检测程序 v6 - SMrz (改进版)')
...
def calculate_percentiles_smoothed(ma_2d, doy_to_indices_for_ref, doy_window=DOY_SMOOTH_WINDOW):
    """
    V6 改进: 使用 DOY 滑动窗口计算平滑的百分位阈值
    每个 DOY 使用 ±doy_window 天的数据，减少阈值锯齿化
    新增 P30 用于滞回恢复判断
    """
    n_lon = ma_2d.shape[1]
    p20_2d = np.full((366, n_lon), np.nan, dtype=np.float64)
    p30_2d = np.full((366, n_lon), np.nan, dtype=np.float64)  # 滞回恢复阈值
    p40_2d = np.full((366, n_lon), np.nan, dtype=np.float64)
    
    for doy in range(1, 366):
        # 收集 doy-window 到 doy+window 的所有索引
        all_indices = []
        ...
        doy_data = ma_2d[combined_indices, :]
...
def main():
    ...
    print("="*70)
    print("   全球干旱检测 v6 - 改进版 (SMrz 根系层土壤湿度)")
    print("="*70)
