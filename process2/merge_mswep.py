...
# 配置
INPUT_DIR = "/data/MSWEP_V315"
OUTPUT_DIR = "/data/MSWEP_V315/yearly"
START_YEAR = 1979
END_YEAR = 2024
...
def read_daily_file(filepath):
    ...
    try:
        with nc.Dataset(filepath, 'r') as ds:
            precip = ds.variables['precipitation'][0, :, :]  # 去掉 time 维度
            return precip
...
def merge_year(year):
    ...
    output_file = os.path.join(OUTPUT_DIR, f"{year}.nc")
    ...
    with nc.Dataset(output_file, 'w', format='NETCDF4') as ds_out:
        ...
        # 创建降水变量 (带压缩)
        precip_var = ds_out.createVariable(
        ...
        # 全局属性
        ds_out.title = f'MSWEP V3.15 Daily Precipitation {year}'
...
def main():
    ...
    print(f"开始合并 MSWEP 数据 ({START_YEAR}-{END_YEAR})...")
