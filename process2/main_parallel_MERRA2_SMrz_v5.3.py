...
# 数据路径配置 - MERRA2 根系层土壤含水量
BASE_DIR = "/home/xulc/flash_drought/gleam"
SM_FILE = "/home/xulc/flash_drought/gleam/result/SMrz_MERRA2_1/SMrz_MERRA2_1980_2024_daily.nc4"
RESULT_DIR = os.path.join(BASE_DIR, "result", "SMrz_MERRA2_1")
SM_VAR = "RZMC"
...
def parse_args():
    parser = argparse.ArgumentParser(description='全球干旱检测程序 v5.3 (5天窗口百分位数) - MERRA2 RZMC')
    ...
    parser.add_argument('--sm-file', type=str, default=None,
                        help='MERRA2 合并后的日尺度土壤湿度文件路径（包含变量 RZMC）')
...
def init_netcdf_files(result_dir, n_lat, n_lon, lat_sub, lon_sub):
    ...
    for event_type in ['total', 'flash', 'nonflash']:
        ...
        with nc.Dataset(filepath, 'w', format='NETCDF4') as ds:
            ...
            ds.source = f'MERRA2 RZMC data ({YEARS[0]}-{YEARS[-1]})'
...
def main():
    ...
    print("="*70)
    print("   全球干旱检测 v5.3 - 5天窗口百分位数优化版 (MERRA2 RZMC 根系层, 日尺度)")
    print("="*70)
