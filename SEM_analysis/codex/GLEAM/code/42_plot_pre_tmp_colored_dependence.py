#!/usr/bin/env python
"""Plot PRE dependence colored by a selected feature for existing SHAP artifacts."""
...
def infer_feature_scale_factor(feature_short: str, values: np.ndarray) -> float:
    return plot_prepeak_vs_recoverywin_comparison.infer_unit_scale_factor(feature_short, values)
...
def build_filtered_frame(
    ...
    pre_col = build_scheme_feature_name(scheme, "total_precipitation_mean")
    color_col = build_scheme_feature_name(scheme, color_feature_short)
    if pre_col not in sample_df.columns or color_col not in sample_df.columns or pre_col not in shap_df.columns:
        return pd.DataFrame(columns=["x", "y", "color"])

    x_raw = pd.to_numeric(sample_df[pre_col], errors="coerce").to_numpy(dtype=float)
    y_raw = pd.to_numeric(shap_df[pre_col], errors="coerce").to_numpy(dtype=float)
...
def plot_one_biome(
    ...
    ax.set_xlim(*xlim)
    ax.set_ylim(*ylim)
    ax.set_xlabel("PRE (mm)", fontsize=11)
    ax.set_ylabel("SHAP value for PRE", fontsize=11)
    ax.set_title(f"{biome} | {title_prefix} PRE dependence colored by {args.color_output_suffix}", fontsize=12)
...
def main() -> None:
    ...
    for biome in biomes:
        ...
        frame = build_filtered_frame(sample_df, shap_df, args.scheme, args.color_feature_short)
        rows.append(
            plot_one_biome(
                biome=biome,
                frame=frame,
                output_path=output_dir / f"{biome}_{args.scheme}_PRE_colored_by_{args.color_output_suffix}.png",
