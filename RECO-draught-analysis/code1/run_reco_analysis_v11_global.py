"""
骤旱对RECO影响分析 - v11 全球版 (180天观测期 + 裁剪版)
==========================================
基于 v10 修改：
  - 输入RECO文件：重采样到0.1°的新文件
  - 输入事件：裁剪后的SMrz事件（已排除沙漠和冰川）
  - WINDOW_AFTER：120天 → 180天（与GPP v11一致）
  - 更好的错误处理
...
def main():
    print("="*70)
    print("骤旱对RECO影响分析 - v11 全球版")
    print("="*70)
    print("数据源:")
    print(f"  RECO: {MERGED_RECO_FILE}")
    ...
    # 保存最终结果
    print("\n保存最终结果...")
    out_file = os.path.join(OUTPUT_DIR, "reco_response_events_global_v11.nc")
    ...
    with nc.Dataset(out_file, 'w') as ds:
        ...
        ds.title = 'RECO Response to SMrz Flash Drought - Global (v11, 180-day window, clipped)'
