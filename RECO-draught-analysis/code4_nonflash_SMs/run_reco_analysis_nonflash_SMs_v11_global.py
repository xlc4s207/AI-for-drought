...
# -*- coding: utf-8 -*-
"""
RECO Response Analysis - Non-Flash Drought (SMs) V11 Global
=============================================================
分析表层土壤湿度 (SMs) 非骤旱干旱事件对 RECO 的影响
...
OUTPUT_DIR = os.path.join(BASE_DIR, "process/RECO-draught-analysis/code4_nonflash_SMs/result")
TEMP_DIR = os.path.join(OUTPUT_DIR, "temp_chunks_nonflash_SMs_reco")
...
@jit(nopython=True)
def process_single_event(reco_z, ws, we, window_before,
                         threshold_resp, threshold_recov, n_consec, max_search):
    """
    处理单个非骤旱干旱事件，分析 RECO 响应
    
    参数:
    - reco_z: z-score 标准化后的 GPP 时间序列 (全时段)
    ...
    return (1, reco_min, reco_mean, reco_trend, t_min_all, t_response,
...
def process_chunk(chunk_info):
    ...
    try:
        ...
        del reco_chunk, ec_chunk, sy_chunk, sd_chunk, ey_chunk, ed_chunk, dur_chunk
        gc.collect()
...
def main():
    print("=" * 70)
    print("GPP Response to Non-Flash Drought (SMs) - V11 Global")
    print("=" * 70)
    print(f"开始时间: {datetime.now()}")
    print(f"\n数据源:")
    print(f"  GPP: {RECO_FILE}")
    ...
    # 保存 NetCDF
    print("\n保存最终结果...")
    out_file = os.path.join(OUTPUT_DIR, "reco_response_nonflash_SMs_drought_v11_global.nc")
...
def save_to_netcdf(results, output_file):
    ...
    with nc.Dataset(output_file, 'w', format='NETCDF4') as ds:
        ...
        ds.variables['reco_min'].long_name = 'Minimum GPP z-score during observation period'
        ds.variables['reco_mean'].long_name = 'Mean GPP z-score during observation period'
        ds.variables['reco_trend'].long_name = 'GPP z-score trend (slope)'
        ...
        # 全局属性
        ds.title = 'GPP Response to Non-Flash SMs Drought - RECO - Global Analysis (v11)'
