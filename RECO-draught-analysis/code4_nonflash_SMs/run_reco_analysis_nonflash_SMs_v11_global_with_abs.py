...
# -*- coding: utf-8 -*-
"""
RECO Response Analysis - Non-Flash Drought (SMs) V11 Global With Absolute Metrics
===============================================================================
在非骤旱分析中同步输出相对值与原始 RECO 绝对值指标。
"""
...
TEMP_DIR = os.path.join(OUTPUT_DIR, "temp_chunks_nonflash_SMs_reco_with_abs")
...
def save_to_netcdf(final_results, out_file):
    with nc.Dataset(out_file, 'w', format='NETCDF4') as ds:
        ...
        ds.title = 'RECO Response to Non-Flash SMs Drought - Global Analysis (v11, relative + absolute metrics)'
...
def main():
    print('=' * 70)
    print('RECO Response to Non-Flash Drought (SMs) - V11 Global (relative + absolute metrics)')
    print('=' * 70)
    ...
    out_file = os.path.join(OUTPUT_DIR, 'reco_response_nonflash_SMs_drought_v11_global_with_abs.nc')
