...
# -*- coding: utf-8 -*-
"""
骤旱RECO响应的季节性分析 (SMs 表层土壤湿度版)
Input: reco_response_SMs_drought_v10_global_merged.nc
"""
import numpy as np
import pandas as pd
...
NC_FILE = os.path.join(BASE_DIR, 'process/RECO-draught-analysis/code2_SMs/results/reco_response_SMs_drought_v10_global_merged.nc')
OUTPUT_DIR = os.path.join(BASE_DIR, 'process/RECO-draught-analysis/code2_SMs/results/seasonal_analysis')
TIF_DIR = os.path.join(OUTPUT_DIR, 'tif')
FIGURE_DIR = os.path.join(OUTPUT_DIR, 'figures')

TARGET_VAR = 'reco_min'  # RECO 分析的目标变量
TARGET_NAME = 'RECO'     # 显示名称
...
def read_and_classify_events():
    ...
    try:
        # 这个merged文件可能有 lat_coord 变量
        lat_data = ds.variables['lat'][:]
        if lat_data.ndim > 1: lat_data = lat_data.flatten()
        
        # 即使是 (event,) 维度的 lat 变量，名字也叫 lat
        # 注意: 之前的 check 脚本显示 keys = ['lat_coord', 'lon_coord', 'lat', 'lon', ...]
        # 'lat' 应该是 event 维度的纬度值
        ...
        data = {
            ...
            'target_min': ds.variables[TARGET_VAR][:].data,
        }
...
def analyze_seasonal_response(df):
    ...
    for season in seasons:
        ...
        stats = {
            ...
            't_recover_mean': season_data['t_recover'].replace(-9999, np.nan).mean(),
