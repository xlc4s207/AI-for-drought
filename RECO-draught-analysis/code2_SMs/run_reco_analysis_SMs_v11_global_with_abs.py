"""
表层土壤湿度 (SMs) 骤旱对 RECO 影响分析 - v11 全球版（相对值 + 绝对值）
=================================================================
...
TEMP_DIR = os.path.join(OUTPUT_DIR, "temp_chunks_SMs_reco_with_abs")
...
def save_to_netcdf(final_results, out_file):
    with nc.Dataset(out_file, 'w') as ds:
        ...
        ds.title = 'RECO Response to SMs Flash Drought - Global (v11, relative + absolute metrics)'
        ds.history = f'Created: {datetime.now()}'
        ds.source_reco = MERGED_GPP_FILE
        ds.source_events = DROUGHT_EVENTS_FILE
        ds.description = 'Relative drought response is detected from RECO z-score anomalies; absolute RECO metrics are computed on the same event window.'


def main():
    print('=' * 70)
    print('表层土壤湿度 (SMs) 骤旱对 RECO 影响分析 - v11 全球版（相对值 + 绝对值）')
    print('=' * 70)
    print('数据源:')
    ...
    out_file = os.path.join(OUTPUT_DIR, 'reco_response_SMs_events_global_v11_with_abs.nc')
    print('\n保存最终结果...')
