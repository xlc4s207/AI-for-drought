...
def process_chunk(chunk_info):
    ...
    try:
        ...
        # 读取 NEE 数据块 (NEE可能是int16类型，需要特殊处理)
        ...
        # 释放所有chunk数据
        del nee_chunk, ec_chunk, sy_chunk, sd_chunk
        gc.collect()
...
def main():
    print("=" * 70)
    print("NEE Response Analysis - SMs Flash Drought Events (V11 Global)")
    print("=" * 70)
    ...
    if len(all_results) > 0:
        ...
        # 保存为 NetCDF
        output_file = os.path.join(OUTPUT_DIR, 'nee_response_SMs_drought_v11_global.nc')
...
def save_to_netcdf(results, lat_arr, lon_arr, output_file):
    ...
    with nc.Dataset(output_file, 'w', format='NETCDF4') as ds:
        ...
        ds.variables['nee_mean'].long_name = 'Mean NEE z-score during response period'
        ds.variables['nee_trend'].long_name = 'NEE z-score trend (slope) during response period'
        ds.variables['t_min'].long_name = 'Days to peak deterioration (max NEE z-score) from drought onset'
        ds.variables['t_response'].long_name = 'Days to first deterioration detection from drought onset'
        ds.variables['t_response'].comment = '-1 means no deterioration detected within search window'
        ...
        # 全局属性
        ds.title = 'NEE Response to SMs Flash Drought Events - Global Analysis (v11)'
        ds.description = ('NEE deterioration under drought is defined as positive NEE anomaly '
                  '(toward carbon source). Recovery is defined as returning below '
