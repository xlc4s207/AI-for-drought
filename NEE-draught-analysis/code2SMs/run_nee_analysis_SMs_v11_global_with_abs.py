"""
表层土壤湿度 (SMs) 骤旱对 NEE 影响分析 - v11 全球版（相对值 + 绝对值）
================================================================
复用原有相对响应判定逻辑（z-score），并在同一次事件分析中输出 NEE 绝对值指标。
"""
...
MERGED_GPP_FILE = "/data/BESS_V2/NEE_1982-2022_0.1deg.nc"
OUTPUT_DIR = os.path.join(BASE_DIR, "process/NEE-draught-analysis/code2SMs/result")
TEMP_DIR = os.path.join(OUTPUT_DIR, "temp_chunks_SMs_nee_with_abs")
...
RESULT_DTYPE = np.dtype([
    ...
    ('recovery_rate', 'f4'),
    ('nee_baseline_abs', 'f4'), ('nee_min_abs', 'f4'), ('nee_max_abs', 'f4'),
    ('nee_mean_abs', 'f4'), ('nee_trend_abs', 'f4'), ('nee_drop_abs', 'f4'),
    ('nee_rise_abs', 'f4'), ('nee_change_to_peak_abs', 'f4'),
    ('nee_recovery_rate_abs', 'f4'),
])
...
def calc_absolute_metrics(gpp_abs, ws, we, t_response, t_impact, t_recover):
    ...
    peak_idx = -1
    if t_response >= 0 and t_impact >= 0:
        peak_idx = t_response + t_impact
    if peak_idx >= 0 and peak_idx < len(post) and np.isfinite(post[peak_idx]):
        peak_val = float(post[peak_idx])
        change_to_peak = peak_val - baseline
        if np.isfinite(t_recover) and t_recover > 0:
            rec_idx = peak_idx + int(round(t_recover))
            if 0 <= rec_idx < len(post) and np.isfinite(post[rec_idx]):
                recovery_rate_abs = (peak_val - float(post[rec_idx])) / float(t_recover)
...
def calc_climatology_zscore(gpp_matrix, doy_idx):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        return (gpp_matrix - full_mean) / full_std
...
def process_chunk(chunk_info):
    ...
    try:
        ...
        gpp_chunk = _gpp_ds.variables['NEE'][:, lat_start:lat_end, :]
        if np.ma.isMaskedArray(gpp_chunk):
            gpp_chunk = gpp_chunk.astype(np.float32)
            gpp_chunk = np.ma.filled(gpp_chunk, np.nan)
        else:
            gpp_chunk = np.asarray(gpp_chunk, dtype=np.float32)
        ec_chunk = _event_ds.variables['event_count'][lat_start:lat_end, :]
        if hasattr(ec_chunk, 'filled'):
            ec_chunk = ec_chunk.filled(0)
        else:
            ec_chunk = np.asarray(ec_chunk)
            ...
            for idx, lon_idx in enumerate(good_lon_indices):
                ...
                for event_id in range(ec):
                    ...
                    if ws < 0 or we >= len(gpp_z):
                        continue
                    rel_metrics = process_single_event_relative(gpp_z, ws, we, THRESHOLD_RESPONSE, THRESHOLD_RECOVER, CONSECUTIVE_DAYS, RESPONSE_SEARCH_WINDOW)
                    abs_metrics = calc_absolute_metrics(gpp_abs, ws, we, int(rel_metrics[5]) if rel_metrics[5] >= 0 else -1, int(rel_metrics[6]) if rel_metrics[6] >= 0 else -1, float(rel_metrics[8]))
                    results.append((
...
def save_to_netcdf(final_results, out_file):
    with nc.Dataset(out_file, 'w') as ds:
        ...
        float_fields = {
            'lat', 'lon', 'nee_min', 'nee_mean', 'nee_trend', 'amp_max', 't_recover', 'recovery_rate',
            'nee_baseline_abs', 'nee_min_abs', 'nee_max_abs', 'nee_mean_abs', 'nee_trend_abs',
            'nee_drop_abs', 'nee_rise_abs', 'nee_change_to_peak_abs', 'nee_recovery_rate_abs',
        }
        ...
        ds.title = 'NEE Response to SMs Flash Drought - Global (v11, relative + absolute metrics)'
        ds.history = f'Created: {datetime.now()}'
        ds.source_nee = MERGED_GPP_FILE
        ds.source_events = DROUGHT_EVENTS_FILE
        ds.description = 'Relative drought response is detected from NEE z-score anomalies; absolute NEE metrics are computed on the same event window.'


def main():
    print('=' * 70)
    print('表层土壤湿度 (SMs) 骤旱对 NEE 影响分析 - v11 全球版（相对值 + 绝对值）')
    print('=' * 70)
    print('数据源:')
    print(f'  NEE: {MERGED_GPP_FILE}')
    ...
    print(f'总事件数(源文件统计): {total_events:,}')
    ...
    all_results = [np.load(os.path.join(TEMP_DIR, tf)) for tf in tqdm(temp_files, desc='合并')]
    final_results = np.concatenate(all_results) if all_results else np.array([], dtype=RESULT_DTYPE)

    out_file = os.path.join(OUTPUT_DIR, 'nee_response_SMs_events_global_v11_with_abs.nc')
    print('\n保存最终结果...')
