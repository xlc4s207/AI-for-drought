...
# -*- coding: utf-8 -*-
"""
NEE Response Analysis - Non-Flash Drought (SMs) V11 Global With Absolute Metrics
==============================================================================
在非骤旱分析中同步输出相对值与原始 NEE 绝对值指标。
"""
...
TEMP_DIR = os.path.join(OUTPUT_DIR, "temp_chunks_nonflash_SMs_nee_with_abs")
...
def calc_absolute_metrics(gpp_abs, ws, we, window_before, t_response, t_impact, t_recover):
        ...
        for rel_lat in range(n_lats):
            ...
            for idx, lon_idx in enumerate(good_lon_indices):
                ...
                for event_id in range(ec):
                    ...
                    abs_metrics = calc_absolute_metrics(gpp_abs, ws, we, WINDOW_BEFORE, int(rel_metrics[5]) if rel_metrics[5] >= 0 else -1, int(rel_metrics[6]) if rel_metrics[6] >= 0 else -1, float(rel_metrics[8]))
                    results.append((
...
def save_to_netcdf(final_results, out_file):
    with nc.Dataset(out_file, 'w', format='NETCDF4') as ds:
        ...
        ds.title = 'NEE Response to Non-Flash SMs Drought - Global Analysis (v11, relative + absolute metrics)'
...
def main():
    print('=' * 70)
    print('NEE Response to Non-Flash Drought (SMs) - V11 Global (relative + absolute metrics)')
    print('=' * 70)
    ...
    out_file = os.path.join(OUTPUT_DIR, 'nee_response_nonflash_SMs_drought_v11_global_with_abs.nc')
