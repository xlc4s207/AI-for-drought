...
# -*- coding: utf-8 -*-
"""
NEE Response Analysis - Non-Flash Drought (SMs) V11 Global
=============================================================
分析表层土壤湿度 (SMs) 非骤旱干旱事件对 NEE 的影响
...
DROUGHT_EVENTS_FILE = os.path.join(BASE_DIR, "gleam/clip_result/SMs_5.3/nonflash_drought_events_v5.nc")
NEE_FILE = "/data/BESS_V2/NEE_1982-2022_0.1deg.nc"
OUTPUT_DIR = os.path.join(BASE_DIR, "process/NEE-draught-analysis/code4_nonflash_SMs/result")
TEMP_DIR = os.path.join(OUTPUT_DIR, "temp_chunks_nonflash_SMs_nee")
...
def process_chunk(chunk_info):
    ...
    try:
        ...
        # 读取 NEE 数据块 (int16 → float32)
        nee_chunk = _nee_ds.variables['NEE'][:, lat_start:lat_end, :]
        if hasattr(nee_chunk, 'mask'):
            nee_chunk = nee_chunk.astype(np.float32)  # 先转换为 float32
            nee_chunk = np.ma.filled(nee_chunk, np.nan)  # 再填充 nan
        else:
        ...
        del nee_chunk, ec_chunk, sy_chunk, sd_chunk, ey_chunk, ed_chunk, dur_chunk
        gc.collect()
...
def main():
    print("=" * 70)
    print("NEE Response to Non-Flash Drought (SMs) - V11 Global")
    print("=" * 70)
    ...
    # 保存 NetCDF
    print("\n保存最终结果...")
    out_file = os.path.join(OUTPUT_DIR, "nee_response_nonflash_SMs_drought_v11_global.nc")
...
def save_to_netcdf(results, output_file):
    ...
    with nc.Dataset(output_file, 'w', format='NETCDF4') as ds:
        ...
        ds.variables['response_detected'].long_name = 'NEE deterioration detected (1=yes, 0=no)'
        ds.variables['nee_min'].long_name = 'Minimum NEE z-score during observation period'
        ds.variables['nee_mean'].long_name = 'Mean NEE z-score during observation period'
        ds.variables['nee_trend'].long_name = 'NEE z-score trend (slope)'
        ds.variables['t_min'].long_name = 'Days to peak deterioration (max NEE z-score) from drought_start'
        ds.variables['t_response'].long_name = 'Days to first deterioration from drought_start'
        ds.variables['t_response'].comment = '-1 means no deterioration detected within observation window'
        ds.variables['t_impact'].long_name = 'Days from deterioration detection to peak deterioration'
        ds.variables['amp_max'].long_name = 'Peak deterioration amplitude (max NEE z-score after response)'
        ds.variables['t_recover'].long_name = 'Days from peak deterioration to recovery'
        ds.variables['recovery_rate'].long_name = 'Rate of recovery from peak deterioration (z-score per day)'
        
        # 全局属性
        ds.title = 'NEE Response to Non-Flash SMs Drought - Global Analysis (v11)'
        ds.description = ('Non-flash drought NEE response analysis with variable observation window. '
                  'Deterioration is defined as positive NEE anomaly (toward carbon source). '
