"""
根系土壤湿度 (SMrz) 骤旱对 NEE 影响分析 - v11 全球版（相对值 + 绝对值）
================================================================
...
OUTPUT_DIR = os.path.join(BASE_DIR, "process/NEE-draught-analysis/code1SMrz/result")
TEMP_DIR = os.path.join(OUTPUT_DIR, "temp_chunks_SMrz_nee_with_abs")
...
def save_to_netcdf(final_results, out_file):
    with nc.Dataset(out_file, 'w') as ds:
        ...
        ds.title = 'NEE Response to SMrz Flash Drought - Global (v11, relative + absolute metrics)'
        ...
        ds.description = (
            'Relative drought response is detected from NEE z-score anomalies. '
            'Absolute NEE metrics are computed from the raw NEE series over the same event window.'
        )


def main():
    print('=' * 70)
    print('根系土壤湿度 (SMrz) 骤旱对 NEE 影响分析 - v11 全球版（相对值 + 绝对值）')
    print('=' * 70)
    print('数据源:')
    ...
    out_file = os.path.join(OUTPUT_DIR, 'nee_response_events_global_v11_with_abs.nc')
    print('\n保存最终结果...')
