#!/usr/bin/env python3
"""Shared compact carbon-response helpers for v20260322_025deg scripts."""
...
def _first_affected(x, start_idx, direction, max_search=None):
    ...
    for i in range(start_idx, n):
        if _is_affected(x[i], direction):
            return i
    return -1


def _baseline_recovery_index(raw_post, start_idx, threshold_value, n_consecutive, direction):
    if raw_post is None:
        return -1
    start_idx = max(0, int(start_idx))
...
def compute_compact_absolute_metrics(segment, window_before, t_peak, metric_name, direction=None):
    direction = infer_direction(metric_name, direction)
...
def compute_event_metrics_from_post(
    ...
    drought_response = _first_affected(post, threshold_offset, direction, max_search=search_len)
    ...
    peak_idx = _find_peak_index(post, drought_response, direction)
    if peak_idx < 0:
        return metrics
    ...
    if metrics["t_recover_to_baseline"] > 0 and np.isfinite(raw_post[peak_idx]):
        if direction == "positive":
            metrics["recovery_rate_to_baseline"] = float(
                (float(raw_post[peak_idx]) - float(baseline)) / metrics["t_recover_to_baseline"]
            )
        else:
            metrics["recovery_rate_to_baseline"] = float(
...
def build_result_dtype(event_mode, metric_name, direction=None):
    names = metric_field_names(metric_name, direction)
    core_fields = [
        ("lat", "f4"),
        ("lon", "f4"),
...
def process_chunk(chunk_info):
    ...
    results = []
    dtype = build_result_dtype(_CONFIG["event_mode"], _CONFIG["metric_name"], _CONFIG["direction"])
    try:
...
def run_relative_analysis(config):
    ...
    dtype = build_result_dtype(config["event_mode"], config["metric_name"], config["direction"])
