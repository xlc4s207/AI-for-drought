...
class TestPrepeakVsRecoverywinComparison(unittest.TestCase):
    ...
    def test_resolve_biomes_falls_back_to_default(self):
        ...
        self.assertEqual(
            biomes,
            ["Forest", "Grassland", "Savanna", "Cropland", "Shrubland", "Wetland"],
        )

    def test_parse_args_defaults_to_5000_beeswarm_points(self):
        with mock.patch.object(sys, "argv", ["prog", "--prepeak-root", "/tmp/pre", "--recoverywin-root", "/tmp/rec", "--output-dir", "/tmp/out"]):
            args = MODULE.parse_args()
        self.assertEqual(args.beeswarm_max_points, 5000)

    def test_format_beeswarm_feature_name_uses_short_labels(self):
        self.assertEqual(MODULE.format_beeswarm_feature_name("prepeak_total_evaporation_mean"), "EVA")
        self.assertEqual(MODULE.format_beeswarm_feature_name("recoverywin_total_precipitation_mean"), "PRE")
        self.assertEqual(MODULE.format_beeswarm_feature_name("prepeak_temperature_2m_mean"), "TMP")
        self.assertEqual(MODULE.format_beeswarm_feature_name("event_onset_days"), "ONS")
        self.assertEqual(MODULE.format_beeswarm_feature_name("event_duration"), "DUR")
        self.assertEqual(MODULE.format_beeswarm_feature_name("event_intensity"), "INT")
