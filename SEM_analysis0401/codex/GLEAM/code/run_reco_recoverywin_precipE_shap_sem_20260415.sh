...
SHAP_GLOBAL_DIR="$RESULT_ROOT/shap_process_recoverywin_precipE"
...
JOBLIB_TMP="/tmp/reco_precipE_recoverywin"
...
mkdir -p "$SHAP_GLOBAL_DIR" "$SHAP_BIOME_DIR" "$SEM_RESULT_ROOT" "$LOG_DIR" "$JOBLIB_TMP"
...
"$PYTHON_BIN" "$SHAP_SCRIPT" \
  ...
  --output-dir "$SHAP_GLOBAL_DIR" >"$LOG_DIR/global_shap.log" 2>&1
...
echo "[DONE] RECO precipE recoverywin SHAP+SEM saved under $RESULT_ROOT"
