#!/usr/bin/env python
"""Legacy-style dependence rebuild for RECO 0401 recoverywin biome SHAP results."""
...
DATASET_CONFIGS: dict[str, DatasetConfig] = {
    "reco0401_recoverywin_mean": DatasetConfig(
        name="reco0401_recoverywin_mean",
        table=PROJECT_ROOT / "data/feature_table_recovery_phase_RECO_code1_flash_SMrz_0401_mswepE.parquet",
        output_root=PROJECT_ROOT / "results/reco_code1_flash_smrz_v20260401_mswepE_clean/shap_process_recoverywin_precipEmean_sample50k_by_biome",
        ...
        metric="RECO",
        biomes=("Cropland", "Forest", "Grassland", "Savanna", "Shrubland", "Wetland"),
    ),
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=["reco0401_recoverywin_mean"],
    )
    parser.add_argument("--biomes", nargs="+", default=None)
    parser.add_argument("--max-missing-rate", type=float, default=0.3)
    parser.add_argument("--random-state", type=int, default=42)
    parser.add_argument("--n-jobs-override", type=int, default=None)
    parser.add_argument("--shap-sample-size-override", type=int, default=None)
...
def fit_and_explain(
    ...
    shap_array = compute_shap_values_parallel(
        model=model,
        sample=sample,
        n_jobs=config.n_jobs,
    )
    shap_df = pd.DataFrame(np.asarray(shap_array), columns=sample.columns, index=sample.index)
    return sample, shap_df, split_metrics, feature_names, len(df)


def _compute_shap_chunk(model, chunk: pd.DataFrame) -> np.ndarray:
...
def compute_shap_values_parallel(
    ...
    if len(sample) == 0:
        return np.empty((0, sample.shape[1]), dtype=float)
    ...
    worker_count = max(1, min(int(requested_jobs), len(sample)))

    if worker_count == 1:
        return _compute_shap_chunk(model, sample)

    chunks = [chunk for chunk in np.array_split(sample, worker_count) if len(chunk) > 0]
    shap_chunks = Parallel(n_jobs=worker_count, prefer="processes")(
        delayed(_compute_shap_chunk)(model, chunk) for chunk in chunks
    )
...
def run_dataset(
    ...
    shap_sample_size_override: int | None = None,
) -> None:
    if n_jobs_override is not None or shap_sample_size_override is not None:
        config = DatasetConfig(
            name=config.name,
            table=config.table,
            ...
            n_estimators=config.n_estimators,
            n_jobs=n_jobs_override if n_jobs_override is not None else config.n_jobs,
            shap_sample_size=(
                shap_sample_size_override
                if shap_sample_size_override is not None
                else config.shap_sample_size
            ),
            include_features=config.include_features,
            exclude_features=config.exclude_features,
...
def main() -> None:
    ...
    for dataset_name in args.datasets:
        run_dataset(
            ...
            n_jobs_override=args.n_jobs_override,
            shap_sample_size_override=args.shap_sample_size_override,
        )
