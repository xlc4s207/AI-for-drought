...
根据骤旱持续期在哪个季节最长来界定该事件的季节属性
分析不同季节的响应特征和空间分布

关键修改：
- 南北半球季节定义相反
- 北半球：春(3-5)、夏(6-8)、秋(9-11)、冬(12-2)
- 南半球：秋(3-5)、冬(6-8)、春(9-11)、夏(12-2)
"""
import numpy as np
import pandas as pd
...
# 南半球季节定义（月份相同但季节相反）
SEASONS_SOUTH = {
    'Autumn': {'months': [3, 4, 5], 'name_zh': '秋季'},    # 对应北半球春季月份
    'Winter': {'months': [6, 7, 8], 'name_zh': '冬季'},    # 对应北半球夏季月份
    'Spring': {'months': [9, 10, 11], 'name_zh': '春季'},  # 对应北半球秋季月份
    'Summer': {'months': [12, 1, 2], 'name_zh': '夏季'}    # 对应北半球冬季月份
}
...
def calculate_drought_season(onset_year, onset_doy, duration, lat):
    """
    计算骤旱的主导季节
    根据骤旱持续期在各季节的天数，选择最长的季节
    
    参数:
        onset_year: 骤旱开始年份
        onset_doy: 骤旱开始的日序（1-366）
        duration: 骤旱持续天数（如t_min或t_min+t_recover）
        lat: 纬度（用于判断南北半球）
    ...
    if duration <= 0 or np.isnan(duration):
        ...
        return get_season_from_month(month, hemisphere)
    
    # 计算骤旱在各季节的天数
    season_days = {season: 0 for season in seasons.keys()}
    ...
    return max(season_days, key=season_days.get)


def calculate_season_for_row(row):
    """
    为单行数据计算季节（用于并行处理）
    
    参数:
        row: DataFrame的一行（Series）
    
    返回:
        季节名称
    """
    return calculate_drought_season(
        int(row['onset_year']), 
        int(row['onset_doy']), 
        row['duration'],
        row['lat']
...
def calculate_season_tuple(args):
    """
    接收元组参数的包装函数（用于并行处理）
    
    参数:
        args: (onset_year, onset_doy, duration, lat)的元组
    
    返回:
        季节名称
    """
    return calculate_drought_season(int(args[0]), int(args[1]), args[2], args[3])
...
def read_and_classify_events():
    ...
    # 计算每个事件的主导季节（使用并行处理）
    n_workers = min(cpu_count(), 32)  # 最多使用32个进程
    print(f"\n正在计算事件的季节属性（考虑南北半球差异）...")
    print(f"使用 {n_workers} 个并行进程处理 {len(df)} 个事件")
    
    # 转换为numpy数组以便更快地传递给子进程
    data_arrays = (
    ...
    # 创建参数元组列表
    print("准备数据...")
    args_list = list(zip(*data_arrays))
    print(f"数据准备完成，共 {len(args_list)} 个任务")
    
    # 使用进程池并行计算（使用更小的chunksize提高并行度）
    chunksize = max(1, len(args_list) // (n_workers * 20))  # 动态计算chunksize
    print(f"chunksize: {chunksize}")
    print("开始并行计算...")
    
    with Pool(processes=n_workers) as pool:
        seasons = list(tqdm(
            pool.imap(calculate_season_tuple, args_list, chunksize=chunksize),
            total=len(args_list),
            desc="计算季节",
            unit="事件",
            smoothing=0.1
...
def create_comparison_figures(df, stats):
    ...
    season_names_zh = [SEASONS_NORTH[s]['name_zh'] for s in seasons]
    
    # 1. 事件数对比
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle('季节性骤旱GPP响应对比分析', fontsize=16, fontweight='bold')
    
    # 1.1 事件数
    ax = axes[0, 0]
    event_counts = [stats[s]['event_count'] for s in seasons]
    colors = ['#2ecc71', '#e74c3c', '#f39c12', '#3498db']
    bars = ax.bar(season_names_zh, event_counts, color=colors, alpha=0.7, edgecolor='black')
    ax.set_ylabel('事件数', fontsize=12)
    ax.set_title('(a) 各季节骤旱事件数', fontsize=12, fontweight='bold')
    ...
    bars = ax.bar(season_names_zh, t_response, yerr=t_response_std, 
                  color=colors, alpha=0.7, edgecolor='black', capsize=5)
    ax.set_ylabel('响应时间 (天)', fontsize=12)
    ax.set_title('(b) 各季节平均响应时间', fontsize=12, fontweight='bold')
    ax.grid(axis='y', alpha=0.3)
    ...
    bars = ax.bar(season_names_zh, t_impact, color=colors, alpha=0.7, edgecolor='black')
    ax.set_ylabel('影响期 (天)', fontsize=12)
    ax.set_title('(c) 各季节平均影响期', fontsize=12, fontweight='bold')
    ax.grid(axis='y', alpha=0.3)
    
    # 1.4 恢复率
    ax = axes[1, 1]
    recovery_rates = [stats[s]['recovery_rate'] for s in seasons]
    bars = ax.bar(season_names_zh, recovery_rates, color=colors, alpha=0.7, edgecolor='black')
    ax.set_ylabel('恢复率 (%)', fontsize=12)
    ax.set_title('(d) 各季节恢复率', fontsize=12, fontweight='bold')
    ax.set_ylim([0, 100])
    ax.grid(axis='y', alpha=0.3)
    for bar in bars:
        height = bar.get_height()
    ...
    fig.suptitle('季节性响应指标分布', fontsize=16, fontweight='bold')
    
    # 2.1 响应时间箱线图
    ax = axes[0]
    data_to_plot = [df[df['season'] == s]['t_response'].values for s in seasons]
    bp = ax.boxplot(data_to_plot, labels=season_names_zh, patch_artist=True)
    for patch, color in zip(bp['boxes'], colors):
        patch.set_facecolor(color)
        patch.set_alpha(0.7)
    ax.set_ylabel('响应时间 (天)', fontsize=12)
    ax.set_title('(a) 响应时间分布', fontsize=12, fontweight='bold')
    ax.grid(axis='y', alpha=0.3)
    ...
    for patch, color in zip(bp['boxes'], colors):
        ...
        patch.set_alpha(0.7)
    ax.set_ylabel('影响期 (天)', fontsize=12)
    ax.set_title('(b) 影响期分布', fontsize=12, fontweight='bold')
    ax.grid(axis='y', alpha=0.3)
    ...
    for patch, color in zip(bp['boxes'], colors):
        patch.set_facecolor(color)
        patch.set_alpha(0.7)
    ax.set_ylabel('恢复时间 (天)', fontsize=12)
    ax.set_title('(c) 恢复时间分布', fontsize=12, fontweight='bold')
    ax.grid(axis='y', alpha=0.3)
