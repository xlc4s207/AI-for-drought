"""
根系土壤湿度 (SMrz) 骤旱对 GPP 影响分析 - v11 全球版（相对值 + 绝对值）
================================================================
...
OUTPUT_DIR = os.path.join(BASE_DIR, "process/GPP-draught-analysis/code1/results")
TEMP_DIR = os.path.join(OUTPUT_DIR, "temp_chunks_SMrz_with_abs")
...
def save_to_netcdf(final_results, out_file):
    with nc.Dataset(out_file, 'w') as ds:
        ...
        ds.title = 'GPP Response to SMrz Flash Drought - Global (v11, relative + absolute metrics)'
        ...
        ds.description = (
            'Relative drought response is detected from GPP z-score anomalies. '
            'Absolute GPP metrics are computed from the raw GPP series over the same event window.'
        )


def main():
    print('=' * 70)
    print('根系土壤湿度 (SMrz) 骤旱对 GPP 影响分析 - v11 全球版（相对值 + 绝对值）')
    print('=' * 70)
    print('数据源:')
    ...
    out_file = os.path.join(OUTPUT_DIR, 'gpp_response_SMrz_events_global_v11_with_abs.nc')
    print('\n保存最终结果...')
