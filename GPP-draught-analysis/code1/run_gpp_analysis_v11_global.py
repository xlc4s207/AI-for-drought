"""
骤旱对GPP影响分析 - v11 全球版 (扩展时间窗口)
==========================================
基于 v10 修改，使用新的骤旱事件文件和扩展时间窗口

v11改进：
  - 使用新的骤旱事件文件 (flash_drought_events_v5.nc)
  - 扩展分析窗口：前60天 + 后180天 (原为后120天)
  - 更长的窗口可以更好地捕捉GPP的完整恢复过程
  - 使用32个worker加速处理

注意：
  - 180天窗口可覆盖约81%事件的完整持续期
  - 2022年7月之后的事件将因数据边界限制无法分析
"""
import os
import numpy as np
...
DROUGHT_EVENTS_FILE = os.path.join(BASE_DIR, "gleam/result/SMrz_result_v5_3/flash_drought_events_v5.nc")
MERGED_GPP_FILE = os.path.join(BASE_DIR, "process/GPP-draught-analysis/SMrz_result/BESS_GPP_1982_2022.nc")
OUTPUT_DIR = os.path.join(BASE_DIR, "process/GPP-draught-analysis/results")
...
def process_chunk(chunk_info):
    ...
    try:
        ...
        # 使用 .filled(-1) 处理masked值，避免int()转换失败
...
def main():
    print("="*70)
    print("骤旱对GPP影响分析 - v11 全球版 (扩展时间窗口)")
    print("="*70)
    print("v11改进：")
    print(f"  - 骤旱事件文件: flash_drought_events_v5.nc")
    print(f"  - 分析窗口: 前{WINDOW_BEFORE}天 + 后{WINDOW_AFTER}天 (原为后120天)")
    print("  - 更长的窗口可以更好地捕捉GPP的完整恢复过程")
    print("")
    print("数据源:")
    print(f"  GPP: {MERGED_GPP_FILE}")
    print(f"  事件: {DROUGHT_EVENTS_FILE}")
    print(f"  Worker数: {N_WORKERS}")
    print("")
    ...
    print("\n保存最终结果...")
    out_file = os.path.join(OUTPUT_DIR, "gpp_response_events_global_v11.nc")
    ...
    with nc.Dataset(out_file, 'w') as ds:
        ...
        ds.title = 'GPP Response to Flash Drought - Global (v11 Extended Window)'
        ds.history = f'Created: {datetime.now()}'
        ds.window_before = WINDOW_BEFORE
        ds.window_after = WINDOW_AFTER
        ds.drought_events_file = 'flash_drought_events_v5.nc'
