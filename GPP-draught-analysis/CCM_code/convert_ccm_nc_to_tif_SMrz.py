"""
Convert CCM NetCDF Results to GeoTIFF (SMrz)
===========================================
将稀疏存储的 CCM 结果 (v4 SMrz) 转换为标准的全球 0.1° GeoTIFF 文件。

输入: ccm_lag_results_v4_SMrz.nc
输出: 
  - ccm_SMrz_lag_star.tif
  - ccm_SMrz_rho_max.tif
  - ccm_SMrz_rho_zero.tif
  - ccm_SMrz_valid.tif
...
INPUT_FILE = os.path.join(BASE_DIR, "process/GPP-draught-analysis/CCM_code/results_v4_SMrz/ccm_lag_results_v4_SMrz.nc")
...
def main():
    ...
    save_tif(grid_lag, "ccm_SMrz_lag_star.tif", rasterio.float32, np.nan)

    # ===== 2. 处理 rho_max =====
    print("\nProcessing rho_max...")
    grid_rho = np.full((ROWS, COLS), np.nan, dtype=np.float32)
    grid_rho[r_idx, c_idx] = rho_max[mask]
    save_tif(grid_rho, "ccm_SMrz_rho_max.tif", rasterio.float32, np.nan)

    # ===== 3. 处理 rho_zero =====
    print("\nProcessing rho_zero...")
    grid_rho_zero = np.full((ROWS, COLS), np.nan, dtype=np.float32)
    grid_rho_zero[r_idx, c_idx] = rho_zero[mask]
    save_tif(grid_rho_zero, "ccm_SMrz_rho_zero.tif", rasterio.float32, np.nan)

    # ===== 4. 处理 valid =====
    print("\nProcessing valid...")
    grid_valid = np.full((ROWS, COLS), 0, dtype=np.int8)
    grid_valid[r_idx, c_idx] = valid[mask]
    save_tif(grid_valid, "ccm_SMrz_valid.tif", rasterio.int8, 0)
    
    print("\nConversion (SMrz) completed successfully!")
