"""
Lagged CCM Analysis V5 - Daily Precision with Coarse-to-Fine Search (SMrz)
==========================================================================
V5 优化版：通过两阶段搜索实现逐日精度的 CCM 分析

优化策略：
1. 粗搜 (Coarse): 每隔 COARSE_STEP 天扫描一次，找到峰值区间
2. 精搜 (Fine): 在峰值区间内逐日扫描，精确定位最佳滞后

计算量对比:
- V4 (step=20): 7 次/像元
- 暴力 (step=1): 121 次/像元
- V5 (Coarse-to-Fine): ~30 次/像元
...
DROUGHT_EVENTS_FILE = os.path.join(BASE_DIR, "gleam/result/SMrz_result/flash_drought_events_details_v2.nc")
GPP_FILE = "/data/BESS_V2/BESS_GPP_1982_2022.nc"
SM_FILE = "/data/GLEAM/SMrz_45years.nc"

OUTPUT_DIR = os.path.join(BASE_DIR, "process/GPP-draught-analysis/CCM_code/results_v5_SMrz")
...
LAG_MAX = 90
COARSE_STEP = 10  # 粗搜步长
FINE_HALF_WINDOW = 9  # 精搜半窗口 (峰值 ± 9 天)
CCM_SUBSAMPLE = 10
MIN_DATA_LENGTH = 300
...
@jit(nopython=True)
def process_pixel_ccm_v5(gpp_anom, sm_anom, E, tau, k, subsample, 
                          lag_min, lag_max, coarse_step, fine_half_window):
    """
    V5 两阶段搜索算法:
    1. 粗搜: 每隔 coarse_step 天扫描
    2. 精搜: 在粗搜峰值附近 ± fine_half_window 内逐日扫描
    """
    # ===== 阶段 1: 粗搜 =====
    n_coarse = (lag_max - lag_min) // coarse_step + 1
    coarse_lags = np.empty(n_coarse, dtype=np.int64)
    coarse_rhos = np.empty(n_coarse, dtype=np.float64)
    
    for i in range(n_coarse):
        coarse_lags[i] = lag_min + i * coarse_step
        coarse_rhos[i] = run_ccm_at_lag_numba(sm_anom, gpp_anom, coarse_lags[i], E, tau, k, subsample)
    ...
    for i in range(n_coarse):
        if not np.isnan(coarse_rhos[i]):
            valid_count += 1
            if coarse_rhos[i] > best_coarse_rho:
                best_coarse_rho = coarse_rhos[i]
                best_coarse_idx = i
    
    if valid_count == 0:
        return (-999, np.nan, np.nan, 0)
    
    center_lag = coarse_lags[best_coarse_idx]
    
    # ===== 阶段 2: 精搜 =====
    fine_start = max(lag_min, center_lag - fine_half_window)
    fine_end = min(lag_max, center_lag + fine_half_window)
    
    best_lag = center_lag
    best_rho = best_coarse_rho
    
    for lag in range(fine_start, fine_end + 1):
        # 跳过已经在粗搜中计算过的点
        if (lag - lag_min) % coarse_step == 0:
            continue
        
        rho = run_ccm_at_lag_numba(sm_anom, gpp_anom, lag, E, tau, k, subsample)
    ...
    # 计算 rho_zero (lag=0)
    rho_zero = np.nan
    if lag_min <= 0 <= lag_max:
        # 检查 0 是否在粗搜中
        if (0 - lag_min) % coarse_step == 0:
            idx_zero = (0 - lag_min) // coarse_step
            rho_zero = coarse_rhos[idx_zero]
        else:
            # 需要单独计算
            rho_zero = run_ccm_at_lag_numba(sm_anom, gpp_anom, 0, E, tau, k, subsample)
    
    return (best_lag, best_rho, rho_zero, 1)
...
def process_single_row(row_idx):
    ...
    try:
        ...
        sm_row = _sm_ds.variables['SMrz'][sm_offset:sm_offset + n_time, row_idx, :]
        ...
        for lon_idx in lon_with_events:
            ...
            lag_star, rho_max, rho_zero, valid = process_pixel_ccm_v5(
                gpp_anom, sm_anom, CCM_E, CCM_TAU, CCM_K, CCM_SUBSAMPLE,
                LAG_MIN, LAG_MAX, COARSE_STEP, FINE_HALF_WINDOW
            )
...
def get_completed_rows():
    ...
    if os.path.exists(TEMP_DIR):
        for f in os.listdir(TEMP_DIR):
            if f.startswith('row_') and f.endswith('.npy'):
                match = re.match(r'row_(\d+)\.npy', f)
                if match:
...
def main():
    print("="*70)
    print("Lagged CCM V5 - Daily Precision with Coarse-to-Fine Search (SMrz)")
    print("="*70)
    print(f"Workers: {N_WORKERS}")
    print(f"CCM参数: E={CCM_E}, τ={CCM_TAU}, K={CCM_K}")
    print(f"Lag范围: [{LAG_MIN}, {LAG_MAX}]")
    print(f"搜索策略: 粗搜步长={COARSE_STEP}, 精搜半窗口=±{FINE_HALF_WINDOW}")
    print("")
    
    # 检查已完成的行
    completed_rows = get_completed_rows()
    print(f"已完成的行数: {len(completed_rows)}")
    ...
    _ = process_pixel_ccm_v5(test_x, test_y, 3, 7, 4, 10, -30, 90, 10, 9)
    print("JIT 预热完成\n")
    ...
    with nc.Dataset(DROUGHT_EVENTS_FILE, 'r') as ds:
        ...
        all_valid_rows = np.where(row_has_events)[0]
        total_pixels_with_events = np.sum(ec_all > 0)
    
    # 过滤掉已完成的行
    remaining_rows = [r for r in all_valid_rows if r not in completed_rows]
    
    print(f"数据总行数 (有事件): {len(all_valid_rows)}")
    print(f"待处理行数: {len(remaining_rows)}")
    ...
    if len(remaining_rows) == 0:
        ...
        with Pool(N_WORKERS, initializer=worker_init) as pool:
            for row_idx, result_arr in tqdm(pool.imap_unordered(process_single_row, remaining_rows),
    ...
    if len(valid_results) > 0:
        ...
        print(f"Lag*范围: [{np.min(lag_stars)}, {np.max(lag_stars)}]")
    
    print("\n保存最终结果...")
    out_file = os.path.join(OUTPUT_DIR, "ccm_lag_results_v5_SMrz.nc")
    ...
    with nc.Dataset(out_file, 'w') as ds:
        ...
        ds.title = 'Lagged CCM V5: SMrz -> GPP (Daily Precision)'
        ds.method = 'CCM with Coarse-to-Fine Search + Numba JIT'
        ds.sm_type = 'SMrz (Root Zone Soil Moisture)'
        ds.ccm_E = CCM_E
        ds.ccm_tau = CCM_TAU
        ds.ccm_k = CCM_K
        ds.lag_range = f"[{LAG_MIN}, {LAG_MAX}]"
        ds.coarse_step = COARSE_STEP
        ds.fine_half_window = FINE_HALF_WINDOW
        ds.lag_precision = "1 day"
