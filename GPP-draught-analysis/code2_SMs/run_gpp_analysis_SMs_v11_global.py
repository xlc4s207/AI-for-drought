"""
表层土壤湿度 (SMs) 骤旱对GPP影响分析 - v11 全球版 (180天观测期 + 裁剪版)
==========================================
基于 v10 修改：
  - 输入文件: 裁剪后的SMs事件（已排除沙漠和冰川）
  - WINDOW_AFTER: 120天 → 180天（与SMrz v11一致）
  - 更好的错误处理
...
def main():
    print("="*70)
    print("表层土壤湿度 (SMs) 骤旱对GPP影响分析 - v11 全球版")
    print("="*70)
    print("数据源:")
    ...
    print("\n保存最终结果...")
    out_file = os.path.join(OUTPUT_DIR, "gpp_response_SMs_events_global_v11.nc")
    ...
    with nc.Dataset(out_file, 'w') as ds:
        ...
        ds.title = 'GPP Response to SMs Flash Drought - Global (v11, 180-day window, clipped)'
