"""
表层土壤湿度 (SMs) 骤旱对 GPP 影响分析 - v11 全球版（相对值 + 绝对值）
================================================================
复用原有相对响应判定逻辑（z-score），并在同一次事件分析中输出 GPP 绝对值指标。
"""
...
TEMP_DIR = os.path.join(OUTPUT_DIR, "temp_chunks_SMs_with_abs")
        ...
        for rel_lat in range(n_lats):
            ...
            for idx, lon_idx in enumerate(good_lon_indices):
                ...
                for event_id in range(ec):
                    ...
                    abs_metrics = calc_absolute_metrics(gpp_abs, ws, we, int(rel_metrics[4]) if rel_metrics[4] >= 0 else -1, float(rel_metrics[8]))
                    results.append((
...
def save_to_netcdf(final_results, out_file):
    with nc.Dataset(out_file, 'w') as ds:
        ...
        ds.title = 'GPP Response to SMs Flash Drought - Global (v11, relative + absolute metrics)'
        ds.history = f'Created: {datetime.now()}'
        ds.source_gpp = MERGED_GPP_FILE
        ds.source_events = DROUGHT_EVENTS_FILE
        ds.description = 'Relative drought response is detected from GPP z-score anomalies; absolute GPP metrics are computed on the same event window.'


def main():
    print('=' * 70)
    print('表层土壤湿度 (SMs) 骤旱对 GPP 影响分析 - v11 全球版（相对值 + 绝对值）')
    print('=' * 70)
    print('数据源:')
    ...
    out_file = os.path.join(OUTPUT_DIR, 'gpp_response_SMs_events_global_v11_with_abs.nc')
    print('\n保存最终结果...')
