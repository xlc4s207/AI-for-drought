...
OUT_DIR = "/home/xulc/flash_drought/process/result_analysis/result_weighted/fluxsat_compare_analysis2/v20260401_growingseason_recovery_gsdays"
SUMMARY_CSV = os.path.join(OUT_DIR, "summary_table_fluxsat_v20260401_growingseason_recovery_gsdays.csv")
SUMMARY_MD = os.path.join(OUT_DIR, "summary_table_fluxsat_v20260401_growingseason_recovery_gsdays.md")
ANNUAL_CSV = os.path.join(OUT_DIR, "annual_response_recovery_trends_fluxsat_v20260401_growingseason_recovery_gsdays.csv")
FIGURE_PATH = os.path.join(OUT_DIR, "fluxsat_gpp_response_recovery_trend_v20260401_growingseason_recovery_gsdays.png")
...
def write_summary_md(rows: List[Dict[str, object]]) -> None:
    ...
    lines = [
        "# FluxSat v20260401 生长季恢复日数版本汇总表",
        "",
        "| " + " | ".join(headers) + " |",
        "| " + " | ".join(["---"] * len(headers)) + " |",
    ]
...
def plot_fluxsat_gpp(annual_rows: List[Dict[str, object]]) -> str:
    ...
    axes[0].set_title("FluxSat GPP 响应时间变化趋势")
    axes[0].set_ylabel("响应时间均值 (天)")
    axes[1].set_title("FluxSat GPP 恢复时间变化趋势")
