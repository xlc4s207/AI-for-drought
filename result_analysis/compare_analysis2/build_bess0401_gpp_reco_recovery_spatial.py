...
"""Build BESS 0401 GPP/RECO recovery-time global spatial maps."""
...
OUT_DIR = (
    ...
    "bess_0401_gpp_reco_recovery_spatial"
)
OUT_PNG = os.path.join(OUT_DIR, "bess_0401_gpp_reco_recovery_mean_global.png")
...
def main() -> None:
    ...
    for ax, panel, data in zip(axes.flat, PANELS, aggregated):
        im = ax.imshow(
        ...
        ax.set_title(f"{panel.row_title} | {panel.col_title}", fontsize=15)
        ax.set_xlabel("Longitude", fontsize=12)
        ax.set_ylabel("Latitude", fontsize=12)
        ax.set_xlim(-180, 180)
        ax.tick_params(labelsize=10)

    fig.suptitle("BESS Recovery Time Mean After Flash Drought", fontsize=20)
