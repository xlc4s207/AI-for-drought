...
BASE_DIR = Path("/home/xulc/flash_drought/era5/clip_result")
...
def plot_maps(lon: np.ndarray, lat: np.ndarray, arrays: dict[tuple[str, str], np.ndarray]) -> Path:
    ...
    for row, soil in enumerate(SOIL_ORDER):
        for col, (event_key, event_cn, cmap_name) in enumerate(EVENT_SPECS):
            ...
            # ERA5 masked fields can be very sparse; add square markers to make valid cells visible.
            if 0 < iy.size <= 5000:
                im = ax.scatter(
                    lon[ix],
                    lat[iy],
    ...
    fig.suptitle("ERA5去除冰原与荒漠后 v5.4 干旱频次分布图", fontsize=18, y=0.98)
    out_path = OUT_DIR / "era5_v54_masked_frequency_maps_2x4.png"
