...
BASE_DIR = Path("/home/xulc/flash_drought/gleam/clip_result")
...
def build_specs() -> list[ScenarioSpec]:
    ...
    for soil in SOIL_ORDER:
        d = BASE_DIR / f"{soil}_result_v5.4_0p25deg_no_ice_desert"
...
def plot_maps(specs: list[ScenarioSpec], lon: np.ndarray, lat: np.ndarray, arrays: dict[tuple[str, str], np.ndarray]) -> Path:
    ...
    fig.suptitle("去除冰原与荒漠后 v5.4 干旱频次分布图", fontsize=18, y=0.98)
    out_path = OUT_DIR / "v54_masked_frequency_maps_2x4.png"
...
def main() -> None:
    ...
    out_path = plot_maps(specs, lon, lat, arrays)
