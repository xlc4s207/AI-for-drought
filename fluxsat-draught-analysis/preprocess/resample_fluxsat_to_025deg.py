...
def create_output_dataset(
    ...
    dst_ds.title = "FluxSat daily GPP 2000-2019 (0.25 degree aligned to GLEAM drought grid)"
    dst_ds.source = "Resampled from FluxSat 0.05 degree lon360 daily file using area-weighted aggregation"
    return dst_ds, {
        "time": time_var,
        "GPP": gpp_var,
...
def resample_file(input_path: str, output_path: str, chunk_size: int, force: bool, complevel: int) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Resample lon360 FluxSat GPP from 0.05 degree to 0.25 degree.")
...
def main() -> None:
    ...
    resample_file(args.input_path, args.output_path, args.chunk_size, args.force, max(0, min(args.complevel, 9)))
